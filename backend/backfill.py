"""
Module for backfilling focal point data for existing images.
"""
...
async def backfill():
    ...
    while True:
        async with SessionLocal() as db:
            result = await db.execute(select(Image).where(Image.focal_point_x == DEFAULT_FOCAL_POINT, Image.focal_point_y == DEFAULT_FOCAL_POINT).limit(batch_size))
